    ...
    <body>
        ...
        <script>
            ...
            $(document).ready(function(){
                ...
                mymap = L.map('mapdiv', {center:[57.04, -3.6], zoom:7, attributionControl:false});
...
//              ******  Load Data  ******
            ...
            function processBUOWL(json, lyr){
                ...
                arHabitatIDs.push(att.id.toString())
            }
            ...
            function styleClientLinears(json) {
                var att = json.properties;
                switch (att.layer) {
                    case 'englandboundary eer':
                        return {color:'peru'};
                        break;
                    case 'sctlandboundary eer':
                        return {color:'navy'};
                        break;
                    case 'northireland wpc':
                        return {color:'navy', dashArray:"5,5"};
                        break;
                    case 'walesboundry eer':
            ...
            function processClientLinears(json, lyr) {
                var att = json.properties;
                lyr.bindTooltip("<h4>Pcode : "+att.layer+"</h4>")
                .bindPopup('<button id="btnPcode">Seen</button>');
            ...
            function refreshLinears() {
                $.ajax({url:'load_allpostcodes.php',
                    data: {tbl:'boundary', flds:"id, layer"},
                    type: 'GET',
                    success: function(response){
                        arProjectIDs=[];
            ...
            function returnEagleMarker(json, latlng){
                ...
                arEagleIDs.push(att.id.toString());
                return L.circle(latlng, {radius:5, color:clrNest,fillColor:'chartreuse', fillOpacity:0.5}).bindTooltip("<h5>Postcode : "+att.field_1+"</h5>Status: "+att.field_2+"<br> Country : "+att.field_5);
            }
            ...
            $("#btnFindEagle").click(function(){
                ...
                if (lyr) {
                    ...
                    lyrSearch = L.circle(lyr.getLatLng(), {radius:10, color:'red', weight:10, opacity:0.5, fillOpacity:0}).addTo(mymap);
            ...
            function refreshEagles() {
                $.ajax({url:'load_pcode.php',
                    data: {tbl:'albansal', flds:"id, field_1, field_2, field_3, field_5 "},
                    type: 'GET',
                    success: function(response){
                        arEagleIDs=[];
